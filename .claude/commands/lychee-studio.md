...
1. **Apply brand.json**: Write `site/custom/brand.json` with brand settings from spec
...
5. **Generate translations**: If additional languages are configured:
   - Read `site/custom/strings/en.json` as the base
   - For each additional language, call the AI API to translate all strings
   - Write translations to `site/custom/strings/{lang}.json`
