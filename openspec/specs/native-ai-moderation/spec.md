...
Forum moderation runs on Run402's platform-native `ai.moderate()` helper, so portal operators supply no AI provider key and an unavailable moderation service never blocks content.
