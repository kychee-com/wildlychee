...
The Silver Pines demo's typed seed module (`src/seeds/silver-pines.ts`) populates a senior-centre portal — warm typography, large base type, and content built around accessibility, transport, health, meals, and technology help.

## Requirements

### Requirement: Silver Pines site configuration
The seed SHALL configure a Silver Pines Senior Center portal with a sage green / cream / amber theme, Merriweather + Source Sans 3 fonts, a larger base font size and generous line height, and feature flags enabling directory, events, announcements, resources, forum, and committees.

#### Scenario: Theme applied on load
- **WHEN** the Silver Pines portal loads
- **THEN** CSS custom properties reflect the sage/cream/amber palette with the seed's enlarged base font size and line height
...
The seed SHALL include 20-25 members with senior profiles: diverse ages (60-85), diverse ethnicities, Asheville-area names, hobbies, volunteer roles, and profile photos. Members SHALL have varied roles (member, volunteer, admin/board).
...
The seed SHALL include 12+ events: 6 upcoming (tai chi, watercolor class, tech help desk, book club, Medicare info session, movie night) and 6 past (garden party, health fair, holiday potluck, nature walk, craft fair, volunteer appreciation). Events SHALL use relative interval expressions for dating.
