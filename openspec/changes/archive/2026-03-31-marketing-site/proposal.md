...
Kychon has a working portal template, a live showcase (Eagles), activity feeds, reactions, and all Phase 1-2 features — but no front door. There's nowhere for a visitor to understand what Kychon is, see it in action, or start using it. The `kychon.run402.com` subdomain currently serves the portal template itself (on the wrong project), not a marketing site.
...
- **Domain setup**: `kychon.com` (Route53) → marketing site. Showcase sites stay on `eagles.run402.com` etc. (or move to `eagles.kychon.com` subdomains later).
